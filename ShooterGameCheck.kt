...
fun main() {
    ...
    repeat(30_000) {
        ...
        if (g.isWiping()) {
            sawWipe = true
            check(g.wipeRow() in 0 until n) { "wipe line off grid: ${g.wipeRow()}" }
            // the sweeping line is lit full-width in the frame
            val row = g.wipeRow()
            check((0 until n).all { x -> ShooterRenderer.toFrame(g)[row * n + x] == 255 }) { "wipe line not full-width" }
        }

        if (!g.isExploding() && !g.isWiping()) {
