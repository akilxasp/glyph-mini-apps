...
class ShooterGame(val n: Int, private val rng: Random = Random.Default) {
    ...
    private var wipeY = -1            // bright line sweeping down after a missile detonates; -1 = idle
    ...
    fun fireMissile() {
        if (missile != null || wipeY >= 0 || blast.exploding || finished) return
    ...
    fun step() {
        ...
        // The wipe owns the board while it sweeps: one row per tick, killing the rank it crosses.
        // Rebuild on the tick it clears the last row, so no frame ever shows an empty board.
        if (wipeY >= 0) {
            for (r in 0 until ROWS) for (c in 0 until COLS) {
                if (alive[r][c] && invaderY(r) == wipeY) alive[r][c] = false
            }
            wipeY++
            if (wipeY >= shipY) { wipeY = -1; if (aliveCount() == 0) newWave() }
            return
        }
    ...
    /** Every MISSILE_EVERY ticks the turret earns one missile: it wipes the whole formation. */
    private fun moveMissile() {
        ...
        m.y--
        val topRow = (0 until ROWS).firstOrNull { r -> (0 until COLS).any { alive[r][it] } }
        if (topRow == null || m.y <= invaderY(topRow)) {
            missile = null
            wipeY = 0            // detonation: a line sweeps from the top down through the ranks
        }
    }
